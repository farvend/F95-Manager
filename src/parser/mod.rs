...
mod tests {
    ...
    // This test exercises decoding an attachment that is served with AVIF content-type,
    // which currently fails without the `image` crate's `avif` feature enabled.
    // It ensures we can reproduce the failure quickly without running the whole app.
    #[tokio::test]
    async fn avif_attachment_decode_fails_currently() {
        // Known attachment that responds with content-type=image/avif (despite .png extension)
        let url = "https://attachments.f95zone.to/2023/10/3018543_f95zone_banner.png";

        // Use the same helper as production code to include Referer and UA
        let res = fetch_image_f95(url).await;
        match res {
            Ok((w, h, _)) => panic!("unexpected AVIF decode success: {}x{}", w, h),
            Err(e) => {
                // Expect a decode error while AVIF support is disabled
                assert!(
                    e.contains("decode error") || e.contains("http status") || e.contains("request error"),
                    "unexpected error: {}",
                    e
                );
            }
    ...
    // This test validates that the server actually responds with AVIF content-type for the URL above,
    // so we know the failure reason is due to decoder support rather than network/headers.
    #[tokio::test]
    async fn avif_attachment_reports_avif_content_type() {
        let url = "https://attachments.f95zone.to/2023/10/3018543_f95zone_banner.png";
        let resp = CLIENT
            .get(url)
