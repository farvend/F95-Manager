...
use crate::app::settings::store::downloaded_game_folder;

#[cfg(target_os = "windows")]
mod win {
    use crate::app::settings::{
        reveal_in_file_manager,
        store::{APP_SETTINGS, downloaded_game_exe, record_downloaded_game},
    };
    ...
    pub(super) fn run_game(dir: &Path, thread_id: u64) {
        let recorded = downloaded_game_exe(thread_id).filter(|p| p.is_file());
        let best = find_exe_closest_to_root(dir);

        let chosen = match (recorded, best) {
            (Some(r), Some(b)) => {
                let rd = depth_from(dir, &r).unwrap_or(usize::MAX);
                let bd = depth_from(dir, &b).unwrap_or(usize::MAX);
                if bd < rd { b } else { r }
            }
            (Some(r), None) => r,
            (None, Some(b)) => b,
        ...
        // Persist the chosen exe (cache or update if changed)
        {
            ...
            if current.as_ref().map(|p| p != &chosen).unwrap_or(true) {
                record_downloaded_game(thread_id, dir.into(), Some(chosen.clone()));
            }
        }
