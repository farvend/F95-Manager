...
pub fn tags_picker(ui: &mut Ui, key: &str, placeholder: &str) -> Option<u32> {
    // Visual constants consistent with other controls
    ...
    let arrow_resp = ui
        .interact(
            arrow_rect,
            ui.id().with("tags_picker_arrow").with(key),
            Sense::click(),
        )
        .on_hover_cursor(eframe::egui::CursorIcon::PointingHand);

    // Open/close popup state
    let popup_id: Id = Id::new(("tags_picker_popup", key));
    let mut is_open = ui
    ...
    if is_open {
        ...
        // Read keys at ui-level so Enter still works when TextEdit just lost focus on this frame.
